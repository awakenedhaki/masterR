...
This is how R knows what you mean when you ask it to give you the mean of some data. It sees `mean` and recognizes that `mean` is a function defined in the base R package. The base R package is imported by default when you run R. All the R objects we define in this session will be stored in `.GlobalEnv`. If you want to know where all these packages and tools can be found in your own computer, use the `searchpaths` function.
...
* What is a relevant name?
  * It is best if you avoid to give your function a name that is already taken, already exists within the _namespace_.
...
Onces you have figured out your function signature, purpose, examples, and a relevant name, you are ready to actually get to some coding. 
