...
The case of our `Income` ourlier and categorical variables, we somehow managed to change those values, fairly easily. The former case, we managed to tell R that we only want the _rows_ that have `Income`'s greater than 0. In the latter case, we selected a column in the dataframe and assigned it new values based on what was already there.

The way you access those segments of your data is through subsetting, which is the topic of our next section.
